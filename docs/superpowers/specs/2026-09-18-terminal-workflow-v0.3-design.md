...
- `Super + Alt + P` as the launcher hotkey;
...
Repository-owned Hyprland Lua registers `Super + Alt + P`.
...
The project intentionally claims `Super + Alt + P` in the managed binding layer. Existing preserved user bindings are loaded first, then this project registers its own launcher binding. The implementation must make the ownership explicit and test it.
...
- `Super + Alt + P`: show/toggle launcher;
...
- `Super + Alt + P` binding registration without using `Super + H`;
...
2. `Super + Alt + P` opens the centered launcher;
...
- `Super + Alt + P` toggles the launcher without blocking Hyprland on discovery;
