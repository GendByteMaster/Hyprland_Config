...
local function assert_click(calls, offset, key)
  local down = calls.dispatches[offset]
  local up = calls.dispatches[offset + 1]
  t.eq(down.key, key)
  t.eq(down.state, "down")
  t.eq(up.key, key)
  t.eq(up.state, "up")
end

t.test("register reserves Num Lock and defines global NumFlow button bindings", function()
...
t.test("NumPad 5 slash star and minus perform fixed mouse clicks", function()
  local hl, o, calls, find = fake_api()
  mouse.register(hl, o, { numlock_store = fake_store(false), hud = fake_hud() })

  find("KP_5", false).dispatcher()
  assert_click(calls, 1, "mouse:272")

  find("KP_Divide", false).dispatcher()
  assert_click(calls, 3, "mouse:272")

  find("KP_Multiply", false).dispatcher()
  assert_click(calls, 5, "mouse:273")

  find("KP_Subtract", false).dispatcher()
  assert_click(calls, 7, "mouse:274")
end)

t.test("NumPad plus always double-clicks left button", function()
  local hl, o, calls, find = fake_api()
  mouse.register(hl, o, { numlock_store = fake_store(false), hud = fake_hud() })
...
t.test("NumPad zero holds left button and decimal releases it", function()
  local hl, o, calls, find = fake_api()
  mouse.register(hl, o, { numlock_store = fake_store(false), hud = fake_hud() })

  find("KP_0", false).dispatcher()
  find("KP_0", false).dispatcher()
  t.eq(#calls.dispatches, 1)
  t.eq(calls.dispatches[1].key, "mouse:272")
  t.eq(calls.dispatches[1].state, "down")

  find("KP_Decimal", false).dispatcher()
  t.eq(#calls.dispatches, 2)
  t.eq(calls.dispatches[2].key, "mouse:272")
  t.eq(calls.dispatches[2].state, "up")
end)

t.test("config reload releases a held left button", function()
  local hl, o, calls, find = fake_api()
  mouse.register(hl, o, { numlock_store = fake_store(false), hud = fake_hud() })
  find("KP_0", false).dispatcher()
  calls.events["config.reloaded"]()
...
t.test("other submaps release a held left button without owning Mouse Mode", function()
  local hl, o, calls, find = fake_api()
  mouse.register(hl, o, { numlock_store = fake_store(false), hud = fake_hud() })
  find("KP_0", false).dispatcher()
