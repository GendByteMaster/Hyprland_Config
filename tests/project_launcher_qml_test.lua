...
t.test("project launcher UI is a hidden floating window with backend processes", function()
  ...
  assert_not_contains(qml, "import QtQuick.Dialogs")
  assert_not_contains(qml, "FolderDialog {")
  assert_contains(qml, "Components.FolderPicker {")
  assert_contains(qml, 'var args = ["browse"]')
  assert_contains(qml, 'rootProcess.exec(root.backendArgs(["add-root", path]))')
  assert_contains(qml, 'text: "Add folder"')
  assert_contains(qml, 'text: "No project folders yet"')
  assert_contains(qml, 'text: "Select folder"')
  assert_contains(qml, "Components.ThemePalette")
  assert_contains(qml, "theme.refresh()")
  assert_contains(qml, "themePalette: theme")
...
t.test("project launcher wrapper uses canonical qs IPC and race-safe bounded startup", function()
  ...
  assert_contains(script, 'OMARCHY_THEME_COLORS')
...
t.test("project launcher visual components are present", function()
  ...
  must_read("quickshell/gendbyte-project-launcher/components/FolderPicker.qml")
  must_read("quickshell/gendbyte-project-launcher/components/StatusMessage.qml")
  must_read("quickshell/gendbyte-project-launcher/components/ThemePalette.qml")
end)


t.test("project launcher folder picker is dark and keyboard navigable", function()
  local picker = must_read("quickshell/gendbyte-project-launcher/components/FolderPicker.qml")
  assert_contains(picker, 'text: "Select project folder"')
  assert_contains(picker, 'text: "Use this folder"')
  assert_contains(picker, "Qt.Key_Backspace")
  assert_contains(picker, "Qt.Key_Left")
  assert_contains(picker, "Qt.Key_Up")
  assert_contains(picker, "Qt.Key_Down")
  assert_contains(picker, "Qt.Key_Return")
  assert_contains(picker, "property var themePalette")
  assert_contains(picker, "root.themePalette.darkBackground")
  assert_contains(picker, "root.themePalette.accent")
  assert_not_contains(picker, "FolderDialog")
end)


t.test("project launcher follows semantic Omarchy theme colors", function()
  local launcher = must_read("quickshell/gendbyte-project-launcher/ProjectLauncher.qml")
  local theme = must_read("quickshell/gendbyte-project-launcher/components/ThemePalette.qml")

  assert_contains(launcher, "theme.background")
  assert_contains(launcher, "theme.foreground")
  assert_contains(launcher, "theme.accent")
  assert_contains(theme, "raw.dark_background || background")
  assert_contains(theme, "raisedSource = raw.lighter_background || raw.selection || foreground")
  assert_contains(theme, "selectionSource = raw.selection || raw.lighter_background || accent")
  assert_contains(theme, "lighterBackground = mix(background, raisedSource")
  assert_contains(theme, "selection = mix(background, selectionSource")
  assert_contains(theme, "function toColor(value, fallback)")
  assert_not_contains(theme, "raw.color8")

  local search = must_read("quickshell/gendbyte-project-launcher/components/SearchField.qml")
  assert_contains(search, "property var themePalette")
  assert_not_contains(search, "property var palette")
  assert_not_contains(search, "palette.accent")
end)

t.test("project launcher keeps partial workspace failures visible", function()
  local launcher = must_read("quickshell/gendbyte-project-launcher/ProjectLauncher.qml")
  assert_contains(launcher, "function formatExecutionSummary(execution)")
  assert_contains(launcher, "payload.data.partial === true")
  assert_contains(launcher, "statusText = formatExecutionSummary(payload.data.execution)")
  assert_contains(launcher, 'item.status === "failed"')
  assert_contains(launcher, "execution.degraded")
  assert_contains(launcher, "item.degraded === true")
  assert_contains(launcher, "placement degraded")
end)
