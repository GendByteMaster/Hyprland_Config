...
t.test("project launcher shell declares stable standalone identity and IPC", function()
  ...
  assert_contains(qml, "LazyLoader {")
...
t.test("project launcher UI is a hidden floating window with backend processes", function()
  ...
  assert_contains(qml, 'backendArgs(["refresh"])')
  assert_contains(qml, 'backendArgs(["query", searchField.text])')
...
t.test("project launcher wrapper uses canonical qs path IPC and bounded startup", function()
  local script = must_read("bin/hyprland-workstation-launcher")
  assert_contains(script, 'qs -p "$CONFIG_DIR" ipc call gendbyte-project-launcher toggle')
  assert_contains(script, 'qs -p "$CONFIG_DIR" ipc call gendbyte-project-launcher ping')
  assert_contains(script, 'qs -p "$CONFIG_DIR" ipc call gendbyte-project-launcher show')
  assert_contains(script, 'HYPRLAND_WORKSTATION_REPO_ROOT')
  assert_contains(script, 'qs -p "$CONFIG_DIR"')
  assert_contains(script, "seq 1 20")
